...
class dataGen2D:
    ...
    def _load_example(cls, frame_path, mask_path, normalize_inputs=True, ignore_last_channel=False):
        frame = cls._load_img(frame_path.numpy().decode("utf-8"), normalize_inputs=normalize_inputs,
                              ignore_last_channel=ignore_last_channel)
        mask = cls._load_img(mask_path.numpy().decode("utf-8"), normalize_inputs=normalize_inputs,
                             ignore_last_channel=False)
        return frame, mask
    ...
    def _load_example_wrapper(self, frame_path, mask_path):
        load_example_partial = partial(self._load_example, normalize_inputs=self.normalize_inputs,
